...
# IAM only allows selected runtime roles to read generated Secrets Manager values.
# Security groups grant network access to RDS/Redis. Database passwords or Redis auth tokens grant application login.
resource "aws_iam_policy" "database_cache_secrets_read" {
  name        = "${local.resource_prefix}-database-cache-secrets-read"
  description = "Allow selected runtime roles to read generated RDS and Redis secrets from Secrets Manager"

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid    = "ReadDatabaseAndCacheSecrets"
        Effect = "Allow"
        Action = [
          "secretsmanager:GetSecretValue",
          "secretsmanager:DescribeSecret"
        ]
        Resource = [aws_secretsmanager_secret.db_password.arn, aws_secretsmanager_secret.redis_auth_token.arn]
      }
    ]
  })
}

resource "aws_iam_role_policy_attachment" "database_cache_secrets_read_ec2_runtime" {
  role       = aws_iam_role.terrapilot_ec2_userdata.name
  policy_arn = aws_iam_policy.database_cache_secrets_read.arn
}

resource "aws_iam_role_policy_attachment" "database_cache_secrets_read_worker_ec2" {
  role       = aws_iam_role.worker_ec2_ecr_pull.name
  policy_arn = aws_iam_policy.database_cache_secrets_read.arn
}
